...
curl -silent -output --header "Authorization: key=$api_key" --header Content-Type:"application/json" https://android.googleapis.com/gcm/send  -d "{\"registration_ids\":[\"$registration_id\"],\"data\":{\"token\":\"$token\",\"message\":\"$message\"}}" 
